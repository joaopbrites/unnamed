<script>
  ...
  async function submit() {
    ...
    const { ok, data } = await api.createProject(payload);
    saving = false;
    if (ok) {
...
<svelte:head><title>Novo Projeto — SDSC Admin</title></svelte:head>

<a href="/admin" class="text-sm text-blue-600 hover:underline mb-4 block">← Painel Admin</a>

{#if admin}
  <h1 class="text-2xl font-bold text-gray-900 mb-6">Novo Projeto</h1>
