...
>
> ✅ **SEGUNDA CAUSA (12/06/2026): login e cadastro falhavam em produção.** Depois do deploy do fix de 404, João reportou que ainda não conseguia logar/cadastrar. Causa: a variável **`VITE_API_URL` no GitHub Actions estava SEM o sufixo `/api`** (`https://unnamed-rzrx.onrender.com` em vez de `.../api`). O bundle publicado chamava `.../token/` (404) em vez de `.../api/token/`. Confirmado inspecionando o JS publicado (`const s="https://unnamed-rzrx.onrender.com"`). **Corrigido no código** (robusto, não depende da config): `api.js` agora tem `normalizeApiBase()` que garante o sufixo `/api` com ou sem ele na variável. `BASE` é exportado e a página `/diagnostics` usa o mesmo BASE do app. (Opcional: também dá pra corrigir a variável no GitHub para `.../api`, mas não é mais necessário.) Requer novo push do `frontend/`.
>
